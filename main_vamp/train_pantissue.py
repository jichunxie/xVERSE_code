...
from main_vamp.utils_model import (
    load_gene_ids,
    MaskFiLMGMMVAE,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the mask-FiLM VampPrior-VAE with original xVERSE data pipeline.")
    ...
    parser.add_argument("--beta-u-kl-multiplier", type=float, default=1.0,
                        help="Extra multiplier on the VampPrior factor u KL inside the KL term. 1.0 keeps standard KL.")
    parser.add_argument("--beta-eps-kl-multiplier", type=float, default=1.0,
                        help="Extra multiplier on the VampPrior residual eps KL inside the KL term. 1.0 keeps standard KL.")
    ...
    parser.add_argument("--prior-cov-rank", type=int, default=0,
                        help="Ignored in main_vamp; kept for CLI compatibility.")
    parser.add_argument("--prior-shared-cov", action="store_true",
                        help="Ignored in main_vamp; VampPrior prior always uses component-specific covariance/factors.")
    ...
    parser.add_argument("--mask-aug-policy", choices=["xverse", "simple"], default="xverse",
                        help="Mask augmentation policy. 'xverse' mimics main_vamp/utils_model.py random masking style.")
    ...
    parser.add_argument("--conditional-prior-on-tissue", action="store_true",
                        help="Use tissue-conditional VampPrior prior p(z|tissue).")
    ...
    parser.add_argument("--prior-logvar-min", type=float, default=-4.0,
                        help="Lower clamp bound for VampPrior prior log-variance used in KL. Prevents tiny prior variance from exploding KL.")
    parser.add_argument("--prior-logvar-max", type=float, default=4.0,
                        help="Upper clamp bound for VampPrior prior log-variance used in KL.")
    ...
    parser.add_argument("--lambda-prior-factor-l2", type=float, default=0.0,
                        help="Weak L2 penalty on VampPrior prior factor loadings to prevent direction variance blow-up.")
    parser.add_argument("--lambda-prior-logvar-l2", type=float, default=0.0,
                        help="Weak L2 penalty that shrinks VampPrior diagonal prior log-variance toward --prior-logvar-target.")
    ...
    parser.add_argument("--prior-init-epoch", type=int, default=3,
                        help="If >0, run one-shot VampPrior prior initialization after this training epoch.")
    parser.add_argument("--prior-init-before-train", action="store_true",
                        help="Run one-shot VampPrior prior initialization from current encoder embeddings before epoch 1.")
    ...
    parser.add_argument("--prior-init-logvar-mode", choices=["cluster", "constant", "shrink"], default="constant",
                        help="How to initialize VampPrior diagonal prior log-variance after delayed k-means init.")
    ...
    parser.add_argument("--prior-init-factor-pca", action="store_true", default=False,
                        help="Initialize VampPrior prior factors from per-cluster local PCA.")
    parser.add_argument("--no-prior-init-factor-pca", dest="prior_init_factor_pca", action="store_false",
                        help="Do not initialize VampPrior factors from local PCA; reset factors to small noise instead.")
    parser.add_argument("--prior-init-factor-scale", type=float, default=0.05,
                        help="Scale applied to PCA-initialized VampPrior prior factors.")
    parser.add_argument("--prior-init-factor-std", type=float, default=0.01,
                        help="Std for small-random VampPrior prior factors when PCA init is disabled.")
...
def main():
    ...
    if args.prior_shared_cov:
        log("[VampPrior] Ignoring --prior-shared-cov: main_vamp has no explicit covariance factors.")
    log(
        f"[VampPrior] pseudo-cell prior: p(z)=sum_k pi_k q_encoder(z|u_k), "
        f"K={args.num_components}; posterior is diagonal q(z|x)."
    )
