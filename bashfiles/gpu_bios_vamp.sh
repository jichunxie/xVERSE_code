...
RESULT_DIR="/hpc/group/xielab/xj58/pretrain_model_celltype/vamp_all_tissue0518_k128_kl1e-4_datapseudo_start2"
...
if [ "${NPROC_PER_NODE}" -le 1 ]; then
  ...
  RUN_CMD=(python -u -m main_vamp.train_pantissue)
else
  echo ">>> Multi-GPU mode: use torchrun"
  RUN_CMD=(torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" -m main_vamp.train_pantissue)
fi
