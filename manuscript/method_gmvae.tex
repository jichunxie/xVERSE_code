...
\subsection{Regularization of the mixture prior}

To avoid degenerate mixture solutions, we optionally apply weak regularization to the
prior. A repulsive center penalty discourages overlapping component centers:
\begin{equation}
    \mathcal{L}_{\mu}
    =
    \frac{1}{K(K-1)}
    \sum_{i\neq j}
    \exp\left(
        -\frac{\|\mu_i-\mu_j\|_2^2}{\eta}
    \right),
\end{equation}
where \(\eta\) controls the range of the repulsion. We also allow a weak log-variance
penalty,
\begin{equation}
    \mathcal{L}_{\sigma}
    =
    ...
    \right\|_2^2,
\end{equation}
which softly encourages component variances to remain near a target value \(a\).
These terms are assigned small weights and are used only to prevent pathological
overlap or overly broad components.

\subsection{Full training objective and model selection}
...
\end{equation}
Different downstream tasks prefer different checkpoints. We therefore save separate
models selected by validation reconstruction loss, validation cell-type loss, and
validation contrastive loss. Embedding analyses use the checkpoint with the best
contrastive loss, as this criterion most directly measures panel-invariant
representation learning. Generative or imputation analyses can instead use the
checkpoint selected by reconstruction likelihood.
