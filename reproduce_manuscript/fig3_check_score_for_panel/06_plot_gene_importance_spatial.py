...
# -*- coding: utf-8 -*-
"""
Plot spatial gene importance evaluation results (Figure 7).
For each spatial dataset, plot Pearson Correlation vs Gene Score Bins (4 bins).
...
SPATIAL_FILE = os.path.join(DATA_DIR, "spatial_gene_importance_corr.csv")
...
BIN_LABELS = ["Top 25%", "25-50%", "50-75%", "Bottom 25%"]
...
def plot_for_bins(df, n_bins):
    ...
    for i, sample in enumerate(valid_samples):
        ...
        if not sample_bench.empty:
            bench_val = sample_bench["MeanCorr"].values[0]
            ...
            for idx, row in sample_trend.iterrows():
                ...
                # Draw vertical line
                ax.vlines(x=bin_x, ymin=val_y, ymax=bench_val, colors='gray', linestyles=':', linewidth=2)
        ...
        # ax.set_ylabel("Mean Pearson Correlation", fontsize=40, labelpad=20) # Removed as requested
    ...
    save_name = f"spatial_gene_importance_combined_{n_bins}bins.png"
    ...
    leg_path = os.path.join(OUTPUT_DIR, f"spatial_gene_importance_legend_{n_bins}bins.png")
...
def main():
    print("Generating Spatial Gene Importance Plots...")
    
    if not os.path.exists(SPATIAL_FILE):
        print(f"Error: File not found {SPATIAL_FILE}")
        return
