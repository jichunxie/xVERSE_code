...
def SpaGE(Spatial_data,RNA_data,n_pv,genes_to_predict=None):
    ...
    Spatial_data_scaled.fillna(0, inplace=True)
    ...
    nbrs = NearestNeighbors(n_neighbors=50, algorithm='auto', metric = 'cosine', n_jobs=-1).fit(Common_data_projected)
    
    # print('Fitting ST data to KNN.')
    distances, indices = nbrs.kneighbors(Spatial_data_projected)

    # print('Start imputation value calculation.')
